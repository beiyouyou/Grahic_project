...
//clampvalue to 0 and 1
void clampValue(double &value)
{
  if (value<0)
    value = 0.0;
  if (value>1)
...
//calculate cross product given two vector
...
double computeVecMagnitude(Vector a)
{
...
//calculating area of triangle in 3d
...
//compute the ray given camera position
void computeRay(int x, int y, Ray &ray, float difX, float difY)
{
  double ratio = (double)(WIDTH)/(double)HEIGHT;
  //make the scene looks bigger
...
//test if the given point is in the triangle
bool pointInTriangleTest(Point c, Triangle triangle)
{
  //three point of triangle
    Point c1, c2, c0;
...
void computeColorForSphere(int index, Ray ray, Point intersectionPoint, Color &color)
{
  ...
  for (int l=0; l<num_lights; l++)
  {
    ...
    for (int i=0; i<num_spheres; i++)
    {
      ...
      {
        //get the intersecting line segment and see if its the incident array
        Vector x,y;
        Point l;
        ...
        if (computeVecMagnitude(x) < computeVecMagnitude(y))
        {
          objectLit = false;
          break;
        }
  ...
  for(x=0; x<WIDTH; x++)
  {
    ...
    //antialiasing method
      if (antialiasing)
      {
        ...
        clampValue(finalColor.r);
        clampValue(finalColor.g);
        clampValue(finalColor.b);

      }
      else
      {
        //calculate the ray from camera to this point
        computeRay(x, y, ray, 0, 0);
